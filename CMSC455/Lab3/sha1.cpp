...
unsigned int func(unsigned int B, unsigned int C, unsigned int D, int t) {
  if(t >= 0 && t <= 19)
  {
	return (B & C) | (~B & D);
  }
...
unsigned int rotate(unsigned int X, unsigned int Y)
{
  return (X << Y) | (X >> (32-Y));
}
...
void sha(block* message, int n, digest& output)
{
		...
		for (unsigned int i=0; i < n; ++i) {
			...
			for(int t = 0; t <= 79; ++t) {
			  unsigned int temp = rotate(A, 5) + func(B, C, D, t) + E + W[t] + K[t];
			  E = D;
			  D = C;
			  C = rotate(B,30);
			  B = A;
